...
// Color sending function declaration
 ...
 * @param c Pointer to SocketClient structure
 * @param color Color string to send (e.g., "red", "green", "blue", "unknown")
 * @return 1 if command sent successfully, 0 if failed
 */
int send_color(SocketClient* c, const char* color) {
    if (!c->running || c->sock == -1 || !color) return 0;
 ...
 * - Response formats: "PICK:True/False", "DROP:True/False", "COLOR:ACK/ERROR"
 */
void* receive_loop(void* arg) {
        ...
        if (n > 0) {
            ...
            for (int i = 0; i < n; i++) {
                if (buffer[i] == '\n') {
                    ...
                    // Check for server responses first
                    if (strncmp(line_buffer, "PICK:", 5) == 0) {
                        printf("🤖 Pick response: %s\n", line_buffer + 5);
                        // No color detection here - pick just returns success/failure
                        ...
                        // Parse sensor data: "S:val1,val2,val3,val4,val5;P:distance;C:r,g,b"
                        char line_copy[2048];
